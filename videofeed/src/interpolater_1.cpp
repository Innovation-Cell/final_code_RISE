...
{	
	...
	{
		for (int j = 0; j < Lane_points[i].size(); ++j)
			temp.at<uchar>(Lane_points[i][j].y, Lane_points[i][j].x) = 255;
	}
	...
	for (int i = 0; i < temp.rows; ++i)
	{
		...
		{
			Final_Grid.data.push_back(temp.at<uchar>(i,j));
		}
	}	

	pub_pix.publish(Final_Grid);
...
int main(int argc, char **argv)
{
	...
	pub_pix = nh.advertise<nav_msgs::OccupancyGrid>("/Lane_Occupancy_Grid", 1);
